...
"""Shared PaliGemma + Gemma-action-expert backbone for the openpi-derived policies.

Historically pi0 and pi05 each carried byte-identical copies of this container (and
pi0_fast a copy of ``GemmaConfig``/``get_gemma_config``); this module is the canonical
home. The class keeps the exact attribute names (``paligemma``, ``gemma_expert``) the
published checkpoints address, so importing it from here has no state_dict impact.
pi0_fast keeps its own ``PI0FastPaliGemma`` variant, whose deltas (openpi rounding
artifact in ``embed_image``, prefix-only forward) are intentional upstream parity.
"""
...
class PaliGemmaWithExpertModel(
    ...
    """PaliGemma VLM with a Gemma action expert, shared by pi0 and pi05."""

...
This module used to be a full vendored fork of ``modeling_qwen2_5_vl.py``. It is now rebased on the
native ``transformers.models.qwen2_5_vl`` classes and only keeps what Wall-X genuinely adds:
...
State-dict layout is unchanged: parameter names and shapes are exactly those of the previous fork, so
the ``x-square-robot/wall-oss-flow`` checkpoint keeps loading through ``Qwen2_5_VLMoEForAction``.
"""
